...
bstack(Monitor *m) 
{
	...
	wl_list_for_each(c, &clients, link) {
		...
		if (i < m->nmaster) {
			...
			resize(c, (struct wlr_box) { .x = m->w.x + mx, .y = m->w.y, .width = w, .height = mh }, 0, draw_borders);
			mx += c->geom.width;
		} else {
			h = m->w.height - mh;
			resize(c, (struct wlr_box) { .x = tx, .y = ty, .width = tw, .height = h }, 0, draw_borders);
			if (tw != m->w.width)
				tx += c->geom.width;
		}
		i++;
...
{
    const Layout **layoutcycle = (const Layout **)arg->v;
